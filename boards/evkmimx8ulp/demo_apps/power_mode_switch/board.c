...
void BOARD_PeripheralReset(void)
{
    rgpio_pin_config_t gpioConfig = {
        kRGPIO_DigitalOutput, /* use gpio as output */
        0U,                   /* output low level */
    };

    RGPIO_PinInit(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_DISPLAY_RESET_PIN)),
                  GPIO_PIN_IDX(BOARD_DISPLAY_RESET_PIN), &gpioConfig);
    SDK_DelayAtLeastUs(20, SystemCoreClock); /* delay 20 us */
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_DISPLAY_RESET_PIN)),
                   GPIO_PIN_IDX(BOARD_DISPLAY_RESET_PIN), 1); /* output high level */

#if 0
    PRINTF("Disable the NFC VEN\r\n");
    RGPIO_PinInit(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_NFC_VEN_PIN)),
                  GPIO_PIN_IDX(BOARD_NFC_VEN_PIN), &gpioConfig);
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_NFC_VEN_PIN)),
                   GPIO_PIN_IDX(BOARD_NFC_VEN_PIN), 0); /* output high level */
    RGPIO_PinInit(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_MIC0_EN_PIN)),
                  GPIO_PIN_IDX(BOARD_MIC0_EN_PIN), &gpioConfig);
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_MIC0_EN_PIN)),
                   GPIO_PIN_IDX(BOARD_MIC0_EN_PIN), 0); /* output high level */
    RGPIO_PinInit(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_MIC1_EN_PIN)),
                  GPIO_PIN_IDX(BOARD_MIC1_EN_PIN), &gpioConfig);
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_MIC1_EN_PIN)),
                   GPIO_PIN_IDX(BOARD_MIC1_EN_PIN), 0); /* output high level */
#endif
    PRINTF("Disable GNSS \r\n");
    RGPIO_PinInit(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_GNSS_RST_PIN)),
                  GPIO_PIN_IDX(BOARD_GNSS_RST_PIN), &gpioConfig);
    SDK_DelayAtLeastUs(20, SystemCoreClock); /* delay 20 us */
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(GPIO_PORT_IDX(BOARD_GNSS_RST_PIN)),
                   GPIO_PIN_IDX(BOARD_GNSS_RST_PIN), 0); /* output high level */
}
