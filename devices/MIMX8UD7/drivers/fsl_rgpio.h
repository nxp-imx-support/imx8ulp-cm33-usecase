...
/*!
 * @brief get gpio port and gpio pin from ioId.
 *
 * such as:
 * io_id = 0x0013, 0x00 is port index(PTA), 0x13 is pin index(19), so it is PTA19.
 */
#define GPIO_PORT_IDX(io_id)          ((uint8_t)(((uint16_t)io_id) >> 8U))
#define GPIO_PIN_IDX(io_id)           ((uint8_t)io_id)
#define GPIO_IO_ID(port_idx, pin_idx) ((uint16_t)(port_idx << 8U | pin_idx))
...
/*!
 * brief Gets RGPIO peripheral base pointer by the RGPIO instance
 *
 * param instance RGPIO instance
 * retval RGPIO peripheral base pointer(PTA, PTB, PTC, etc.)
 */
RGPIO_Type *RGPIO_GetBaseByInstance(uint32_t instance);
/*@}*/
