...
#define MODE_CFG_SHUTDOWN_MASK  (1 << 7)
...
#define TEMP_CONFIG_TEMP_EN_MASK (1 << 0)
...
typedef enum _max_sample_average
{
    kMAX_SMP_AVE_1 = 0x00,
    kMAX_SMP_AVE_2 = 0x01,
    kMAX_SMP_AVE_4 = 0x02,
    kMAX_SMP_AVE_8 = 0x03,
    kMAX_SMP_AVE_16 = 0x04,
    kMAX_SMP_AVE_32 = 0x05
} max_sample_average_t;

/*! @brief Define I2C access function. */
typedef status_t (*I2C_SendFunc_t)(uint8_t deviceAddress, uint32_t subAddress, uint8_t subaddressSize, uint32_t txBuff, uint32_t flags);
typedef status_t (*I2C_ReceiveFunc_t)(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subaddressSize, uint8_t *rxBuff, uint8_t rxBuffSize, uint32_t flags);
...
typedef struct _max_config_t
{
    ...
    max_sample_average_t sampleavg;
} max_config_t;

typedef struct _max_sample_t
{
    uint8_t sample_data[MAXIM_BYTES_PER_ADC_VALUE];
} max_sample_t;

typedef struct _max_sample_buf_t
{
    union
    {
        max_sample_t sample_buf[MAX_CFG_TOTAL_SAMPLES + MAXIM_FIFO_DEPTH]; /* Add FIFO depth, in case of overflow */
        uint8_t sample_data[(MAX_CFG_TOTAL_SAMPLES + MAXIM_FIFO_DEPTH) * MAXIM_BYTES_PER_ADC_VALUE];
    };
