...
uint32_t HAL_RtcGetEnabledInterrupts(hal_rtc_handle_t halRtcHandle)
{
    ...
    if (bbnsm_flags & kBBNSM_RTC_AlarmInterrupt)
        flags |= kHAL_RTC_AlarmInterrupt;
...
uint32_t HAL_RtcGetStatusFlags(hal_rtc_handle_t halRtcHandle)
{
    ...
    if (bbnsm_flags & kBBNSM_RTC_AlarmInterruptFlag)
        flags |= kHAL_RTC_AlarmInterruptFlag;
...
void HAL_RtcClearStatusFlags(hal_rtc_handle_t halRtcHandle, uint32_t flags)
{
    ...
    if (flags & kHAL_RTC_AlarmInterruptFlag)
        bbnsm_flags |= kBBNSM_RTC_AlarmInterruptFlag;
...
void HAL_RtcEnableInterrupts(hal_rtc_handle_t halRtcHandle, uint32_t flags)
{
    ...
    if (flags & kHAL_RTC_AlarmInterrupt)
        bbnsm_flags |= kBBNSM_RTC_AlarmInterrupt;
