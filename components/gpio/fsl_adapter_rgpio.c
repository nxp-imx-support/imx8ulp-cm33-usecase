...
typedef struct _hal_gpio_pin
{
    ...
    uint16_t trigger : 4U;
    uint16_t sel : 3U; /* kRGPIO_InterruptOutput0, kRGPIO_InterruptOutput1, kRGPIO_InterruptOutput2,
                          kRGPIO_InterruptOutput3 */
...
void HAL_GpioInterruptHandle(uint8_t instance)
{
    ...
    while (NULL != head)
    {
        if (head->pin.port == instance)
        {
            portIntFlags =
                RGPIO_GetPinsInterruptFlags(RGPIO_GetBaseByInstance(instance), (rgpio_interrupt_sel_t)head->pin.sel);
            if (0U != (portIntFlags & (1UL << head->pin.pin)))
            {
                ...
                RGPIO_ClearPinsInterruptFlags(RGPIO_GetBaseByInstance(instance), (rgpio_interrupt_sel_t)head->pin.sel,
                                              portIntFlags);
            }
        }
...
hal_gpio_status_t HAL_GpioInit(hal_gpio_handle_t gpioHandle, hal_gpio_pin_config_t *pinConfig)
{
    ...
    gpioState->pin.irq       = pinConfig->irq;

    switch (pinConfig->sel)
    {
        case kHAL_GpioInt0:
            gpioState->pin.sel = kRGPIO_InterruptOutput0;
            break;
        case kHAL_GpioInt1:
            gpioState->pin.sel = kRGPIO_InterruptOutput1;
            break;
        case kHAL_GpioInt2:
            gpioState->pin.sel = kRGPIO_InterruptOutput2;
            break;
        case kHAL_GpioInt3:
            gpioState->pin.sel = kRGPIO_InterruptOutput3;
            break;
        default:
            break;
    }
    ...
    RGPIO_PinInit(RGPIO_GetBaseByInstance(gpioState->pin.port), gpioState->pin.pin, &gpioPinConfig);
...
hal_gpio_status_t HAL_GpioGetInput(hal_gpio_handle_t gpioHandle, uint8_t *pinState)
{
    ...
    *pinState = (uint8_t)RGPIO_PinRead(RGPIO_GetBaseByInstance(gpioStateHandle->pin.port), gpioStateHandle->pin.pin);
    return kStatus_HAL_GpioSuccess;
}
...
hal_gpio_status_t HAL_GpioSetOutput(hal_gpio_handle_t gpioHandle, uint8_t pinState)
{
    ...
    RGPIO_PinWrite(RGPIO_GetBaseByInstance(gpioStateHandle->pin.port), gpioStateHandle->pin.pin,
                   (0U != pinState) ? 1U : 0U);
    return kStatus_HAL_GpioSuccess;
}
...
hal_gpio_status_t HAL_GpioSetTriggerMode(hal_gpio_handle_t gpioHandle, hal_gpio_interrupt_trigger_t gpioTrigger)
{
    ...
    RGPIO_SetPinInterruptConfig(RGPIO_GetBaseByInstance(gpioStateHandle->pin.port), gpioStateHandle->pin.pin,
                                (rgpio_interrupt_sel_t)gpioStateHandle->pin.sel, triggerConfig);
    if (kRGPIO_InterruptOrDMADisabled != triggerConfig && gpioStateHandle->pin.irq != NotAvail_IRQn)
    {
#ifdef GPIO6_0_IRQn
        NVIC_EnableIRQ(Rgpio_IRQn[gpioStateHandle->pin.port]);
#else
        NVIC_EnableIRQ(gpioStateHandle->pin.irq);
#endif
    }
